...
function ToggleRow({ label, options, value, onChange }: {
  ...
  return (
    <div>
      <p className="text-[10px] uppercase tracking-[0.18em] text-faint mb-2">{label}</p>
      <div className="flex gap-2">
        {options.map(([v, l]) => (
          <button
            key={v}
            onClick={() => onChange(v === 'true' ? true : v === 'false' ? false : v)}
            className={`flex-1 py-3 rounded text-xs font-plex tracking-widest uppercase transition-colors ${
              String(value) === v
  ...
  return (
    <div className="h-screen bg-bg flex flex-col">
      ...
      <div className="flex-1 flex min-h-0">
        {/* Left: user info */}
        <div className="flex-[3] flex flex-col justify-center px-10 border-r border-border-subtle">
          <p className="text-xs uppercase tracking-[0.2em] text-faint mb-1">Ciao,</p>
          <h1 className="font-display italic text-5xl text-ink mb-3 truncate">{user}</h1>
        ...
        {/* Right: controls */}
        <div className="flex-[2] flex flex-col justify-center gap-4 px-8">
          <ToggleRow
            label="Shot type"
          ...
          <button
            ...
            className="py-3 rounded text-xs uppercase tracking-[0.16em] text-faint border border-border bg-transparent"
          >
            Logout
          </button>
        </div>
